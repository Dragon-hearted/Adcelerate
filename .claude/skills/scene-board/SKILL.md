...
| **4.5** | **Character Sheet (optional)** | **If ≥2 protagonists detected: generate one composite reference sheet per character (6 poses, white studio backdrop) for downstream scene consistency** |
...
When a script has ≥2 protagonists, text descriptions alone aren't enough — characters drift between scenes (wrong jaw, wrong hair, wrong jacket). Stage 4.5 fixes this by generating a single composite reference sheet per character before scene generation starts.
...
7. **Thread** each character's single composite sheet into every downstream scene's `referenceImageIds` (via `resolveReferenceImageIds()` in `batch-generator.ts`). NanoBanana Pro reads whichever panel in the composite matches the scene's camera — no per-scene view selection is needed.

**Four-tier consistency model** (updated from the legacy three-tier):

| Tier | Signal | Handled by |
|---|---|---|
| 0 | Character reference sheet (per-scene pixel anchor) | Stage 4.5 output → `resolveReferenceImageIds()` (one sheet per character) |
...
| 3 | Scene-1 environment anchor (pixel continuity for setting) | `dependsOn` chain — **dropped when character sheets fill the 3-ref cap** |

<template>
  <Teleport to="body">
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 mobile:p-0">
      ...
      <!-- Modal -->
      <div
        class="relative bg-[var(--theme-bg-primary)] rounded-lg mobile:rounded-none shadow-xl flex flex-col overflow-hidden z-10 mobile:w-full mobile:h-full mobile:fixed mobile:inset-0"
        :style="{ width: '85vw', height: '85vh' }"
        :class="{ 'mobile:!w-full mobile:!h-full': true }"
        @click.stop
      >
          <!-- Header -->
          <div class="flex-shrink-0 bg-[var(--theme-bg-primary)] border-b border-[var(--theme-border-primary)] p-6 mobile:p-3">
            <div class="flex items-center justify-between mb-4 mobile:mb-2">
              <h2 class="text-3xl mobile:text-lg font-semibold text-[var(--theme-text-primary)]">
                💬 Chat Transcript
              </h2>
              <button
                @click="close"
                class="p-2 mobile:p-3 hover:bg-[var(--theme-hover-bg)] rounded-lg transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
              >
                <svg class="w-6 h-6 mobile:w-5 mobile:h-5 text-[var(--theme-text-tertiary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-4">
              ...
              <div class="flex gap-2">
                <div class="relative flex-1">
                  <input
                    ...
                    class="w-full px-4 py-2 mobile:px-3 mobile:py-2 pl-10 mobile:pl-8 text-lg mobile:text-base border border-[var(--theme-border-secondary)] rounded-lg focus:ring-2 focus:ring-[var(--theme-focus-ring)] focus:border-[var(--theme-focus-ring)] bg-[var(--theme-bg-primary)] text-[var(--theme-text-primary)]"
                  >
                  <svg class="absolute left-3 top-3 w-5 h-5 text-[var(--theme-text-quaternary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                  </svg>
                </div>
                <button
                  @click="executeSearch"
                  class="px-4 py-2 mobile:px-3 mobile:py-2 bg-[var(--theme-primary)] hover:bg-[var(--theme-primary-hover)] text-white font-medium rounded-lg transition-colors text-base mobile:text-sm min-w-[44px] min-h-[44px] flex items-center justify-center"
                >
                  Search
                </button>
                <button
                  @click="copyAllMessages"
                  class="px-4 py-2 mobile:px-3 mobile:py-2 bg-[var(--theme-bg-quaternary)] hover:bg-[var(--theme-bg-tertiary)] text-[var(--theme-text-primary)] font-medium rounded-lg transition-colors text-base mobile:text-sm min-w-[44px] min-h-[44px] flex items-center justify-center"
              ...
              <!-- Filters -->
              <div class="flex flex-wrap gap-2 mobile:gap-1 max-h-24 mobile:max-h-32 overflow-y-auto p-2 mobile:p-1 bg-[var(--theme-bg-secondary)] rounded-lg mobile:overflow-x-auto mobile:pb-2">
                <button
                ...
                <button
                  ...
                  @click="clearSearch"
                  class="px-4 py-2 mobile:px-3 mobile:py-1.5 rounded-full text-sm mobile:text-xs font-medium bg-[var(--theme-accent-error)]/20 text-[var(--theme-accent-error)] hover:bg-[var(--theme-accent-error)]/30 min-h-[44px] mobile:min-h-[36px] flex items-center whitespace-nowrap"
                >
                  Clear All
                </button>
              </div>
              ...
              <!-- Results Count -->
              <div v-if="activeSearchQuery || activeFilters.length > 0" class="text-sm mobile:text-xs text-[var(--theme-text-tertiary)]">

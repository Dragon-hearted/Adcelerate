// PORTED VERBATIM from apps/server/src/transcript-ingest.ts. The inode/offset
// rotation handling, the per-file mutex + tombstone shutdown protocol, and the
// byte-exact (never re-encoded) offset advance are all load-bearing and subtle;
// preserve exactly. Only the `./db` import was re-pointed to `./store` (the
// ported persistence layer) — behaviour is identical.
...
/**
 * Decide whether a watched path should be ignored. We only want to skip
 * dot-files / dot-dirs that live INSIDE the watched tree — never reject the
 * whole tree because the watch ROOT itself sits under a dot-directory.
 *
 * BUGFIX (#2): this was ported verbatim from apps/server as
 * `ignored: /(^|[/\\])\../`, which tests the FULL ABSOLUTE path. Since the root
 * is `~/.claude/projects`, the `.claude` ancestor matched the regex for the
 * root and every descendant — so chokidar ignored the ENTIRE tree and ingested
 * zero transcripts (token_events never populated). Relativizing to the root so
 * only dot-segments *within* the tree are excluded fixes it.
 */
export function isIgnoredTranscriptPath(rootDir: string, p: string): boolean {
  const rel = path.relative(rootDir, p);
  if (rel === '') return false; // the watch root itself
  if (rel.startsWith('..')) return false; // outside the root — not our concern
...
export async function startTranscriptIngest(opts: IngestOptions = {}): Promise<() => Promise<void>> {
  const rootDir = path.resolve(opts.rootDir ?? PROJECTS_DIR);
  ...
  const watcher = chokidar.watch(rootDir, {
    ...
    // Ignore dot-files/dirs INSIDE the tree only — relative to the root, so the
    // root's own dot-ancestry (e.g. ~/.claude/projects) doesn't nuke the watch.
    ignored: (p: string) => isIgnoredTranscriptPath(rootDir, p),
  });

// PORTED WHOLESALE from apps/server/src/token-queries.ts. The local-timezone
// period/bucket math (DST-safe via Date#setDate) and the split-SUM + COALESCE
// NULL-safety are non-obvious and correct — preserve EXACTLY.
//
// The only changes from the legacy file:
//   • the SQL runs against the raw `bun:sqlite` handle (`sqlite`, aliased to
//     `db` below) that the Drizzle client wraps — same connection, same DB,
//     Drizzle-generated schema. SQL strings are byte-identical to the legacy
//     implementation so the `/api/tokens/*` JSON stays parity-compatible.
//   • the response/param types are imported from `@command-center/shared`
//     (the single cross-package contract) instead of re-declared, so the
//     orchestrator and web client cannot drift.
//
// TIME ORIGIN CHOICE
// ------------------
...
// Re-exported under the legacy names so the route layer reads identically to
// apps/server/src/routes/tokens.ts.
export type Range = TokenRange;
export type Bucket = TokenBucket;
export type { BreakdownDimension, PeriodTotals, SummaryResponse, TimeseriesPoint, BreakdownRow };
